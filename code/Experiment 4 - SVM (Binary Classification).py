...
ss = StandardScaler()
ss.fit(data.drop('Purchased', axis =1 ))#先删除（drop）购买与否的数据再进行拟合数据和标准化处理
...
#7，调用SVC库
from sklearn.svm import SVC #SVC支持向量机用于分类，如果用于回归则是SVR
svc = SVC()
svc.fit(x_train,y_train)
predictions = svc.predict(x_test)

#8，得出结果
from sklearn.metrics import classification_report , confusion_matrix #引用混淆矩阵进行评估，二分类，训练集，测试集
print(confusion_matrix (y_test , predictions)) #混淆矩阵
print(classification_report(y_test , predictions)) #不用print，则返回结果为一行，\n划分间隔。查准率=0.96，查全率=0.95 准确率=0.94
print(svc.score(x_train, y_train))# 训练集准确率 0.9000
print(svc.score(x_test, y_test)) #  测试集准确率 0.9416